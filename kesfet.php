...
<div class="container-fluid">
	<div class="row-fluid">
		...
		function convertDate($date_){
			...
			$query_aa = $db->prepare("SELECT * FROM takipler WHERE eden_id IN (".$userId.")");
			$query_aa->execute();
			while($row_aa = $query_aa->fetch(PDO::FETCH_ASSOC)){
				$aa_id[] = $row_aa['edilen_id'];
			}
			$aa_userId = implode(',',$aa_id);
			if(!isset($_GET['hashtag']) || $_GET['hashtag'] == null){
				$query = $db->prepare("SELECT p.*,k.*,b.*,h.* FROM paylasilanlar as p,kisiler as k,begeniler as b,hashtag as h WHERE (k.id = p.kisi_id) AND (p.kisi_id IN (".$aa_userId.") AND p.kisi_id <> :id) GROUP BY p.p_id ORDER BY p.p_id DESC");
			}else{
				$hashtag = $_GET['hashtag'];
				$query = $db->prepare("SELECT p.*,k.*,b.*,h.* FROM paylasilanlar as p,kisiler as k,begeniler as b,hashtag as h WHERE (k.id = p.kisi_id AND h.gonderi_id = p.p_id) AND (h.hashtag IN ('".$hashtag."')) GROUP BY p.p_id ORDER BY p.p_id DESC");
			}
			$query->bindParam(':id',$id);
			$query->execute();
